...
tem = model.Model(params,device = torch.device("cpu") )
print(tem)
...
filt_size = 41
plt.figure()
plt.plot(analyse_cpu.smooth(np.mean(np.array([env for env_i, env in enumerate(correct_model) if envs_to_avg[env_i]]),0)[1:], filt_size), label='tem')
