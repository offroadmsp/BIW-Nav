...
class Model(torch.nn.Module):
    ...
    def init_iteration(self, g, x, a, M):
        ...
        device = next(self.parameters()).device  # 获取模型所在的设备
        g_inf = [torch.stack([self.g_init[f] for _ in range(self.hyper['batch_size'])], device=device) for f in range(self.hyper['n_f'])]
            ...
            for a_i, a in enumerate(prev_iter[0].a):
                ...
                if a is None:
                    ...
                    for f, g_inf in enumerate(prev_iter[0].g_inf):
                        g_inf[a_i,:] = self.g_init[f].to(device)
    ...
    def gen_g(self, a_prev, g_prev, locations):
        ...
        device = next(self.parameters()).device  # 获取模型所在的设备
        g = [mu_g[f] + sigma_g[f] * torch.randn_like(mu_g[f], device=device) if self.hyper['do_sample'] else mu_g[f] for f in range(self.hyper['n_f'])]
    ...
    def gen_p(self, g, M_prev):
        ...
        device = next(self.parameters()).device  # 获取模型所在的设备
        p = [mu_p[f] + sigma_p[f] * torch.randn_like(mu_p[f], device=device) if self.hyper['do_sample'] else mu_p[f] for f in range(self.hyper['n_f'])]
    ...
    def f_c(self, decompressed):
        ...
        device = next(self.parameters()).device  # 获取模型所在的设备
        return torch.stack([self.hyper['two_hot_table'][i] for i in torch.argmax(decompressed, dim=1)], dim=0).to(device)
    ...
    def f_mu_g_path(self, a_prev, g_prev, no_direc=None):
        ...
        if self.hyper['has_static_action']:
            ...
            a_prev_tensor = torch.tensor(a_prev_step, device=device)
            a = torch.zeros((len(a_prev_step),self.hyper['n_actions']), device=device).scatter_(1, torch.clamp(a_prev_tensor.unsqueeze(1)-1,min=0), 1.0*(a_prev_tensor.unsqueeze(1)>0))
        else:
            # Without static actions: each action label should become a one-hot vector for that label
            a_prev_tensor = torch.tensor(a_prev_step, device=device)
            a = torch.zeros((len(a_prev_step),self.hyper['n_actions']), device=device).scatter_(1, a_prev_tensor.unsqueeze(1), 1.0)
...
class LSTM(torch.nn.Module):
    ...
    def prepare_data(self, data_in):
        ...
        device = data_in[0][1].device  # 获取数据所在的设备
...
class Iteration:
    ...
    def correct(self):
        ...
        device = self.x.device  # 获取数据所在的设备
