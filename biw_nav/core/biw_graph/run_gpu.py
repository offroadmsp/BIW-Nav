...
if load_existing_model:
    ...
    date = '2024-12-22' # 2020-07-05 run 0 for successful node agent
    run = '1'
    i_start = 0 # the default starting iteration for the loaded model, 40
    ...
    model_bak = importlib.util.module_from_spec(model_spec)
    model_spec.loader.exec_module(model_bak)
    
    # Load the parameters of the model
    params = torch.load(model_path + '/params_' + str(i_start) + '.pt', map_location=device)
    ...
    tem = model_bak.Model(params).to(device)  # 移动到设备
    tem.hyper['device'] = device
    # Load the model weights after training
    model_weights = torch.load(model_path + '/tem_' + str(i_start) + '.pt', map_location=device)
...
for i in range(i_start, params['train_it']):
    ...
    # print('Starting backprop iteration {:d}/{:d}'.format(i, params['train_it']))
    ...
    for env_i, walk in enumerate(walks):
        # print('Environment {:d}/{:d}'.format(env_i+1, len(walks)))
    ...
    for i_step, step in enumerate(chunk):
        chunk[i_step][1] = torch.stack(step[1], dim=0).to(device)  # 移动到设备
    
    # 如果prev_iter不为None，将其移动到设备
    if prev_iter is not None:
        prev_iter = [step.detach().to(device) for step in prev_iter]
    
    # Forward-pass this walk through the network
    forward = tem(chunk, prev_iter)    
    
    # Accumulate loss from forward pass
    loss = torch.tensor(0.0, device=device)  # 在设备上创建损失张量
    ...
    for step in forward:            
        ...
        for env_i, env_visited in enumerate(visited):
            if env_visited[step.g[env_i]['id']]:
                step_loss.append(loss_weights.to(device)*torch.stack([l[env_i] for l in step.L]))
            else:
        ...
        plot_loss = plot_loss + step_loss.detach().cpu().numpy()  # 移动到CPU进行numpy操作
    ...
    if i % 10 == 0:
        ...
        print("start output log\n")
        print(logger.level)  # 应该 <= 20 (INFO级别)
        logger.info('Finished backprop iter {:d} in {:.2f} seconds.'.format(i,time.time()-start_time))
        logger.info('Loss: {:.2f}. <p_g> {:.2f} <p_x> {:.2f} <x_gen> {:.2f} <x_g> {:.2f} <x_p> {:.2f} <g> {:.2f} <reg_g> {:.2f} <reg_p> {:.2f}'.format(loss.detach().cpu().numpy(), *plot_loss))  # 移动到CPU
        logger.info('Accuracy: <p> {:.2f}% <g> {:.2f}% <gt> {:.2f}%'.format(acc_p, acc_g, acc_gt))
        logger.info('Parameters: <max_hebb> {:.2f} <eta> {:.2f} <lambda> {:.2f} <p2g_scale_offset> {:.2f}'.format(np.max(np.abs(prev_iter[0].M[0].detach().cpu().numpy())), tem.hyper['eta'], tem.hyper['lambda'], tem.hyper['p2g_scale_offset']))  # 移动到CPU
        ...
        writer.add_scalar('Losses/Total', loss.detach().cpu().numpy(), i)  # 移动到CPU
