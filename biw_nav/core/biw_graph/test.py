...
date = '2024-12-22' # 2020-10-13 run 0 for successful node agent
run = '1'
index = '39000'
...
tem = model.Model(params, device = 'cpu' )
# print(tem)
...
# print('Environments:', environments[0])
# print('envs:', envs)
...
correct_model, correct_node, correct_edge = analyse_cpu.compare_to_agents(forward, tem, environments, include_stay_still=include_stay_still)

# Analyse occurrences of zero-shot inference: predict the right observation arriving from a visited node with a new action
zero_shot = analyse_cpu.zero_shot(forward, tem, environments, include_stay_still=include_stay_still)

# Generate occupancy maps: how much time TEM spends at every location
occupation = analyse_cpu.location_occupation(forward, tem, environments)

# Generate rate maps
g, p = analyse_cpu.rate_map(forward, tem, environments)

# Calculate accuracy leaving from and arriving to each location
from_acc, to_acc = analyse_cpu.location_accuracy(forward, tem, environments)
...
plt.plot(analyse_cpu.smooth(np.mean(np.array([env for env_i, env in enumerate(correct_model) if envs_to_avg[env_i]]),0)[1:], filt_size), label='BIW')
plt.plot(analyse_cpu.smooth(np.mean(np.array([env for env_i, env in enumerate(correct_node) if envs_to_avg[env_i]]),0)[1:], filt_size), label='node')
plt.plot(analyse_cpu.smooth(np.mean(np.array([env for env_i, env in enumerate(correct_edge) if envs_to_avg[env_i]]),0)[1:], filt_size), label='edge')
plt.ylim(0, 1)
plt.legend()
# 添加x轴标签
...
plt.title('Zero-shot inference: ' + str(np.mean([np.mean(env) for env_i, env in enumerate(zero_shot) if envs_to_avg[env_i]]) * 100) + '%', font={'family':'Arial', 'size':10})
plt.tight_layout()
# plt.show()
plt.savefig('./Pics/fig_infer1.png', dpi=600)  # 保存图像为PNG格式，设置dpi为300
...
plot.plot_map(environments[env_to_plot], np.array(to_acc[env_to_plot]), ax1)
ax1.set_title('Accuracy when arriving location')
ax2 = plt.subplot(1,2,2)
plot.plot_map(environments[env_to_plot], np.array(from_acc[env_to_plot]), ax2)
ax2.set_title('Accuracy when leaving location')
plt.title('Location accuracy')
plt.savefig('./Pics/fig_location1.png', dpi=600)  # 保存图像为PNG格式，设置dpi为300
'''
# 创建画布和子图
...
# 设置整体标题（可选，放在顶部居中）
fig.suptitle('Location Accuracy', font={'family':'Arial', 'size':10})  # y 稍微上移避免与子图标题冲突
# 自动调整子图间距，避免重叠
plt.tight_layout()
# 保存图像
plt.savefig('./Pics/fig_location1.png', dpi=600, bbox_inches='tight')  # bbox_inches 避免裁剪
...
# plt.savefig('./Pics/fig_map1.png', dpi=600)  # 保存图像为PNG格式，设置dpi为300
# plt.figure(figsize=(3.5, 2.2),dpi=600)
...
# plt.savefig('./Pics/fig_occupation1.png', dpi=600)  # 保存图像为PNG格式，设置dpi为300
plt.show()
