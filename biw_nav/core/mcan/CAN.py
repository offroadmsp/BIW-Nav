...
class attractorNetwork:
    ...
    def update_weights_dynamics(self,prev_weights, delta, moreResults=None, cross=None):
        ...
        full_shift_amount=lambda x: np.floor(x) if x > 0 else np.ceil(x)    # 计算偏移的位置
        full_shift[(non_zero_idxs + int(full_shift_amount(delta)))%self.N]=prev_weights[non_zero_idxs]*self.activity_mag    # 计算某个位置的权重
        ...
        '''excitation'''
        excitations_store=np.zeros((len(shifted_indexes),self.N))   # 存储激励位置
