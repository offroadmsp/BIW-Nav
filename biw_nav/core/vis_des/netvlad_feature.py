...
import sys
sys.path.append('./ratslam')
sys.path.append('vis_des')
import ratslam

parser = argparse.ArgumentParser(description='vis_des')
parser.add_argument('--mode', type=str, default='feature', help='Mode', choices=['train', 'test', 'cluster'])
...
parser.add_argument('--dataPath', type=str, default='/home/zhen/cogMap/vis_des/data/', help='Path for centroid data.')
parser.add_argument('--runsPath', type=str, default='/home/zhen/cogMap/vis_des/runs/', help='Path to save runs to.')
parser.add_argument('--savePath', type=str, default='checkpoints', 
        help='Path to save checkpoints to in logdir. Default=checkpoints/')
parser.add_argument('--cachePath', type=str, default='/media/zhen/Share/temp/', help='Path to save cache to.')
...
def feature_extract(epoch=0, write_tboard=False):
    ...
    print('===> Running feature extract step')
    epoch = 1

    opt.cacheBatchSize = 1
    ...
    with torch.no_grad():
        ...
        dbFeat = np.empty((len(whole_test_set), pool_size))
        model = model.cuda()
        ...
        for iteration, (input, indices) in enumerate(test_data_loader, 1):
            ...
            # print("x:",slam.experience_map.current_exp.x_m, "y: ", slam.experience_map.current_exp.y_m)
    ...
    dbFeat = dbFeat[:whole_test_set.dbStruct.numDb].astype('float32')
    print('====> Feature extraction complete')

    # dbFeat = feature_extract(whole_test_set, epoch, write_tboard=False)
