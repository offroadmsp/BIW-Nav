...
Author:   Chi Chung Alan Fung (Modified for Cognitive Relation Ablation)
...
+ A \exp\left[-\frac{\left|x-z_0\right|^2}{4a^2}\right] + I_{relation}(x,t)
...
class cann_model:
    ...
    def __init__(self, argument):
        ...
        # [Added for Ablation Study] Initialize relation input (default is 0)
        self.relation_input = np.zeros((self.N)); 
    ...
    # [Added for Ablation Study] Function to set cognitive relation bias
    def set_relation_input(self, bias_vector):
        """
        Set the cognitive relation bias input (I_relation).
        This is used to test the impact of cognitive relations in ablation studies.
        
        Args:
            bias_vector (np.array or None): The bias input vector. 
                                            If None, resets to zero (ablation).
        """
        if bias_vector is None:
            self.relation_input = np.zeros((self.N))
        else:
            if bias_vector.shape != self.input.shape:
                raise ValueError(f"Shape of relation input mismatch! Expected {self.input.shape}, got {bias_vector.shape}")
            self.relation_input = bias_vector
    ...
    def get_dudt(self, t, u):
        # [Modified for Ablation Study] Added self.relation_input to the dynamics
        dudt = \
        -u + np.dot(self.Jxx, self.cal_r_or_u(u)) * self.dx + self.input + self.relation_input;
        dudt = dudt / self.tau;
        return dudt
